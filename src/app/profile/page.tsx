...
import { BookmarkSkeleton, FavoriteSkeleton, ReadingHistorySkeleton } from '@/components/SkeletonComponents';
...
export default function ProfilePage() {
  ...
  const getNeededSurahIds = () => {
    if (activeTab === 'bookmarks' && bookmarks) {
      return [...new Set(bookmarks.map(bookmark => bookmark.surah_id))];
    }
    if (activeTab === 'favorites' && favorites) {
      return [...new Set(favorites.map(favorite => favorite.surah_id))];
    }
    if (activeTab === 'notes' && userNotes) {
      return [...new Set(userNotes.map(note => note.surah_id))];
    }
    return [];
  };
                    ...
                    <div className="space-y-3 animate-fadeIn">
                      {Object.entries(
                        ...
                        .map(([surahId, surahBookmarks]) => {
                          ...
                          const sortedBookmarks = [...surahBookmarks].sort((a, b) => a.ayat_number - b.ayat_number);
