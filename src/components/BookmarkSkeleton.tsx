...
export default function BookmarkSkeleton({ count = 3, groupsCount = 2 }) {
            ...
            {Array.from({ length: groupSize }).map((_, i) => (
              <div key={`item-${groupIndex}-${i}`} className="p-3 flex justify-between items-center">
                <div className="flex items-center">
                  ...
                  <div>
                    <div className="h-4 bg-amber-50 rounded w-24 mb-2"></div>
                    <div className="h-3 bg-amber-50 rounded w-40"></div>
                  </div>
                </div>
