...
export default function SearchInput({ 
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Get current query value based on whether we're controlled or not
    ...
    <form onSubmit={handleSubmit} className="mb-6">
      <div className="relative">
        ...
        {showResults && searchResults && searchResults.length > 0 && (
          <div className="absolute w-full mt-2 bg-white dark:bg-gray-800 border border-amber-200 dark:border-amber-700/50 rounded-lg shadow-lg z-20 max-h-[60vh] overflow-y-auto">
            {searchResults.map((result) => (
              <Link 
                key={result.key} 
                href={result.href}
                onClick={() => setShowResults(false)}
                className="block p-3 sm:p-4 border-b border-amber-100 dark:border-amber-700/30 last:border-b-0 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors"
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-sm sm:text-base font-bold text-amber-900 dark:text-amber-200">{result.title}</div>
                  <div className="text-xs sm:text-sm text-amber-600 dark:text-amber-400">{result.surah}</div>
                </div>
                <div className="text-xs sm:text-sm text-amber-700 dark:text-amber-300/70 mt-1">{result.snippet}</div>
              </Link>
            ))}
          </div>
        )}
